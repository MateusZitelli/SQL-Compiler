...
public class Identifier {
    ...
    private static int actualInt = null;
    private static double actualDouble = null;

    public static Token identifyToken(string input){
        char[] palavra = input.toLowerCase().toCharArray();
        ArrayList<Character> command = new ArrayList<Character>();

        for(char letra : palavra){
            findNumber(letra);
            findReservedWord(letra);
        }
    }

    public static Token updateNumber(Character letra) {
        // Ir verificando carácter à carácter 
    }

    private static Token updateReservedWord(Character letra) {
        switch (state) {
            case 0:
                switch (letra) {
                    case '(':
